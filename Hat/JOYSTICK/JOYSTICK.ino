...
int plats[20] = {0,   0, 25,  0, 50,  0, 75,  0, 100, 0,
...
void loop() {
    ...
    if (!dead) {
        for (int i = 0; i < 20; i += 2) {
            ...
            M5.Lcd.drawLine(plats[i + 1], plats[i], plats[i + 1] + platwidth,
                            plats[i], GREEN);
            //    EsploraTFT.stroke(150, 89, 43);
            M5.Lcd.drawLine(plats[i + 1], plats[i] - 1,
                            plats[i + 1] + platwidth, plats[i] - 1, BLACK);
            plats[i] = plats[i] + 1;
            if (plats[i] > 239) {
                // sets the platform to the top of screen
                plats[i] = 0;
                // sets random x position
                plats[i + 1] = (random(109)) % 109;
            }
        }
        step++;
        int tilt = x_data;
        ...
        for (int i = 0; i < 20; i += 2) {
            if (yPos >= plats[i] - 3 && prevyPos < plats[i] &&
                xPos < plats[i + 1] + platwidth && xPos > plats[i + 1]) {
                yPos       = plats[i] - 3;
                upvelocity = 4.5f;
            }
        }
